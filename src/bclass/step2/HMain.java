...
public class HMain extends JFrame implements ActionListener {
	...
	public HMain(String title) {//인스턴스화를 할 때마다 반복해서 호출이 된다.
		this.title = title;//전변 초기화를 하였다
	}	
...
2. A a = null;
    a = new A();
3. A a = B.getInstance();  싱글톤패턴 - 하나다 - static대신에 써라
4. Super s = new Sub(); - 다형성
인터페이스 s  = new 구현체 클래스()
 추상클래스 s = new 구현체 클래스()    
 
 인터페이스는 기능적인 관점에서 결합도 낮추면서 재정의를 하여 사용하는 컨벤션
 
 추상클래스(생성자도 가능, 일반메소드도 가능하다)
 :서비스의 확장과 관련된 관점
 :추상클래스는  extends사용하므로 인터페이스 처럼 결합도가 낮지 않고 오히려 강함 - 재사용성 떨어짐(트러블)
 
 카카오 -> 카카오 톡 -> 카카오 선물 -> 카카오 페이 -> 카카오 뱅크
 전혀 다른 서비스
 
 분석 - 설계 - 개발 - 테스트 - 배포 : 폭포수 모델

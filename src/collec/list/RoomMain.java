...
public class RoomMain {
	...
	public void room2() {
		r2 = new Room();
		r2.inwon = 20;
		r2.title = "보드 같이 타실분 모집-달방구함";
		r2.names.add("나신입");
		r2.names.add("나일등");
		r2.names.add("이성계");
	}
	public static void main(String[] args) {
		// insert here - Spring에서는 개발자가 직접 인스턴스화 하지 않는다.
		// 필요할 때 Spring이 대신(주도권이 넘어갔다) 객체를(자동) 주입해준다.
		// 객체가 생성되는 시점에 따라 문제가 발생할 수  있다.(객체 라이프사이클-> 탄생-활동-죽음(소멸-캔디데이트상태{가비지컬렉터-자원회수})
		// 이른(미리, 먼저해둔다.전변성격)인스턴스화, 게으른 인스턴스화(지역적으로 처리한다.) 나눈다.

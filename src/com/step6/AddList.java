 ...
 * 키를 컬럼명으로 통일하면 일관성있는 변수명으로 접근 가능함 - 속도도 빠름
 * 
 */
class Emp1{
	private int empno = 0;
...
public class AddList {
	...
	public static void main(String[] args) {
		...
		//기존에 코드를 변경해야 한다. - 있는걸 바꾼다 -  사이드이펙트 (부수효과)발생함
		//이식성, 재사용성 - 객체지향 언어가 추구하는 방향성
		//전역변수 - 클래스 중심의 코딩을 전개하기 - 중요함 - 유지됨 - 람다식과 같은 함수형에서는 사용하지 않는다.-왜냐면 사이드이펙트
		//지역변수 - 처리하면 그 메소드안에서만 유지됨 - 사이드이펙트 발생하지않음
		//안전함 - 람다식 - 함수형 프로그래밍 지원 가능한 이유
		//상속받거나 구현체 클래스를 선언할 때 하위 클래스를 상속 받는 게 더 많은 변수와 메소드를 사용할 수 있다.
		...
		//Map은 객체타입으로 받기때문에 그 값을 가지고 연산을 할 때는 형전환을 해야함(CastingException이 다수 발생가능함)
		//제네릭 타입에는 멀티 타입을 적을 수 없어서 2개 이상의 집합을 조인했을 경우 VO타입보다는 Map을 사용하는 것이 
		//복잡도를 줄일 수 있다.
		List<Map<String,Object>> list  = new ArrayList<>();// []형식을 가짐 (Gson. lib 데이터 수집, 형전환, 변환) - JSON

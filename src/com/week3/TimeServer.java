...
//main스레드와 통신객체 사이에 경합이 벌어질 수 있어서 굳이 Thread상속 받음 - 안정화(메신저)
public class TimeServer extends Thread {
	//아래 소켓은 서버소켓에 접속해온 클라이언트의 소켓정보를 쥐고 있다.
	//TimeClient에서 new Socket(ip,port);호출될 때 그때 결정되므로 일단 null이다.
	Socket client = null;
	...
	public static void main(String[] args) {
		...
		while(true) {
			try {
				//클라이언트 소켓이 접속해 올 때까지 기다리고 있다. - waiting상태 다음 코드가 진행되지 않음
				client = server.accept();
				if(client !=null) {
					//System.out.println(client);
				}
				System.out.println("New client connected...");
				//전변으로 선언된 client가 69번라인에 실체가 생김

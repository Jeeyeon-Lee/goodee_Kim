...
//데이터셋을 담당하는 DefaultTableModel API활용하기
...
public class EmpManager extends JFrame implements ActionListener{
	...
	//데이터 셋이다. 화면이 아니다.
	DefaultTableModel dtm = new DefaultTableModel(data, header);// web개발 -> JSON역할이됨-실제 데이터를 쥐고 있다.
	...
	public void initDisplay() {
		jbtn_update.addActionListener(this);//this- 자바스크립트 죽음 -
		jp_north.setLayout(new GridLayout(1,4));//default  FlowLayout이다 - 왼쪽에서 부터 오른쪽으로 배치된다. 그리고 공간이 남음
		...
		System.out.println(data[1][1]);//나초보
		System.out.println(data[1][2]);//4000
		System.out.println(data[2][0]);//7748
	...
	@Override
	public void actionPerformed(ActionEvent e) {//콜백메소드의 파라미터 자리는 JVM이 대신 주입(인스턴스화)해줌 -> arrow function
		Object obj = e.getSource();
		if(jbtn_update == obj) {//너 수정버튼 누른거야?
			//insert here 4000-> 5000조정해주세요.
			...
			for(int i=0;i<dtm.getRowCount();i++) {//데이터셋은 테이블(로우+컬럼) 구조이다.
				...
				if(jt.isRowSelected(i)) {//너가 선택한 로우값은???? - true, 만일 선택된 로우가 아니면 false
					//나초보랑 같은 로우값이라면
					//나초보를 가져오는 낱말카드 - String name = dtm.getValueAt(1,1);//나초보가 담김
					//if("나초보"==name)
					String cname = (String)dtm.getValueAt(i, 1);
					System.out.println("사용자가 선택한 로우의 이름은 ===>"+cname);
					if("나초보"==cname) {//여기가 나초보에 해당되는 로우값이라면
						dtm.setValueAt(5000, i, 2);
					}// end of if					
					else if("나신입".equals(cname)) {//여기가 나초보에 해당되는 로우값이라면
						dtm.setValueAt(3500, i, 2);
					}// end of if		
					else if("나일등".equals(cname)) {//여기가 나초보에 해당되는 로우값이라면

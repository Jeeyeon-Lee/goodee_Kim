...
public class BMain implements ActionListener{
	JFrame jf = new javax.swing.JFrame();//자바최초 MVC패턴 API적용한 사례
	B1 b1 = new B1();//위치, 유지(쿠키(로컬),세션(서버:캐쉬메모리)-시간지배함), 가치, 활용범위(기초가 아니다) - 사이드 프로젝트 - 꾸준히 관찰하기
	B2 b2 = null;//선언만 - 생성자 호출은 없다
	BMain(){
		b2 = new B2();//생성하기-생성자 경유한다.
		/*
		for(double d=0.0;d<9999999.0;d++) {
	...
	public void initDisplay() {
		//B1 b1 = new B1();
		//이벤트 소스와 이벤트 처리를 담당하는 클래스 연결하기 - 콜백메소드(시스템이 자동으로 호출함-시점결정권개발자에게 없다) 호출됨
